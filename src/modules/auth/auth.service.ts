...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<LoginResponseDto> {
    ...
    if (user.role === UserRole.STUDENT) {
      enrolledCoursesCount = await this.enrollmentModel
        .countDocuments({ userId: user._id, status: EnrollmentStatus.ACTIVE })
        .exec();
    }
  ...
  /* ─────────────────────────────
       Find User by ID
    ───────────────────────────── */
  async findUserById(userId: string): Promise<JwtPayload> {
